					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<form action="<?php echo site_url('currencies/search');?>" id="currencies-form" name="currencies-form" class="form-horizontal" method="post" accept-charset="utf-8" novalidate="novalidate">
											...
											<div class="row mb-2">
												...
												<div class="col-8">
													<select class="form-control form-control-sm select2bs4 col-12" id="status" name="status">
														...
														<option value="<?php echo STATUS_INACTIVE;?>"><?php echo $this->lang->line('status_suspend');?></option>
													</select>
												</div>
											</div>
											<div class="row mb-2">
												<label class="col-4 col-form-label col-form-label-sm font-weight-normal"><?php echo $this->lang->line('label_currency_code');?></label>
												<div class="col-8">
													<input type="text" class="form-control form-control-sm" id="currency_code" name="currency_code" value="">
												</div>
											</div>										
										</div>
										<div class="col-md-3">
							...
							<!-- /.card-header -->
							<?php if(permission_validation(PERMISSION_CURRENCIES_ADD) == TRUE):?>
							<div class="card-header">
							...
							<?php endif;?>
							<div class="card-body">
								<table id="currencies-table" class="table table-striped table-bordered table-hover" style="width:100%;">
									<thead>
										<tr>
											<th><?php echo $this->lang->line('label_hashtag');?></th>
											<th><?php echo $this->lang->line('label_currency_name');?></th>
											<th><?php echo $this->lang->line('label_currency_code');?></th>
											<th><?php echo $this->lang->line('label_currency_symbol');?></th>
											<th><?php echo $this->lang->line('label_transfer_rate');?></th>
											<th><?php echo $this->lang->line('label_transfer_fee');?></th>
											<th><?php echo $this->lang->line('label_deposit_rate');?></th>
											<th><?php echo $this->lang->line('label_deposit_fee');?></th>
											<th><?php echo $this->lang->line('label_withdrawal_rate');?></th>
											<th><?php echo $this->lang->line('label_withdrawal_fee');?></th>
											...
											<?php if(permission_validation(PERMISSION_CURRENCIES_UPDATE) == TRUE OR permission_validation(PERMISSION_CURRENCIES_DELETE) == TRUE):?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#currencies-table').DataTable({
				...
				"order": [[0, "desc"]],
				"ajax": {
					"url": "<?php echo site_url('currencies/listing');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function addData() {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_add_currencies');?>',
				content: '<?php echo base_url('currencies/add/');?>'
			});
		}
		...
		function updateData(id) {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_currencies_setting');?>',
				content: '<?php echo base_url('currencies/edit/');?>' + id
			});
		}
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('currencies/delete/');?>' + id, function(data) {
					var json = JSON.parse(JSON.stringify(data));
