				...
				<div class="row">
					<div class="col-12">
						<div class="card">
									...
									<div class="form-group row">
										<div class="col-md-3">
											<div class="row mb-2">
												...
												<div class="col-8">
													<input type="text" class="form-control form-control-sm" id="name" name="name" value="<?php echo (isset($data_search['name']) ? $data_search['name'] : '');?>">
												</div>
											</div>
											<div class="row mb-2">
							...
							<?php endif;?>
							<div class="card-body">
								<table id="user_role-table" class="table table-striped table-bordered table-hover" style="width:100%;">
									<thead>
										<tr>
	...
	<script type="text/javascript">
		$(document).ready(function() {
					...
					$.ajax({url: form.attr('action'),
						...
						success: function (data) {
							...
							if(json.status == '<?php echo EXIT_SUCCESS;?>') {
								$('#user_role-table').DataTable().ajax.reload();
							}
							else {
			...
			$('#user_role-table').DataTable({
				...
				"order": [[0, "desc"]],
				"ajax": {
					"url": "<?php echo site_url('role/listing');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function addData() {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_add_user_role');?>',
				content: '<?php echo base_url('role/add/');?>'
			});
		}
		...
		function updateData(id) {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_user_role_setting');?>',
				content: '<?php echo base_url('role/edit/');?>' + id
			});
		}
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('role/delete/');?>' + id, function(data) {
					var json = JSON.parse(JSON.stringify(data));
