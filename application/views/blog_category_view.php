		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="row">
					<div class="col-12">
						<div class="card">
							<?php if(permission_validation(PERMISSION_BLOG_CATEGORY_ADD) == TRUE):?>
							...
							<?php endif;?>
							<div class="card-body">
								<table id="blog_category-table" class="table table-striped table-bordered table-hover" style="width:100%;">
									<thead>
										<tr>
											...
											<?php if(permission_validation(PERMISSION_BLOG_CATEGORY_UPDATE) == TRUE OR permission_validation(PERMISSION_BLOG_CATEGORY_DELETE) == TRUE):?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#blog_category-table').DataTable({
				...
				"order": [[0, "desc"]],
				"ajax": {
					"url": "<?php echo site_url('blog/listing_category');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function addData() {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_add_blog_category');?>',
				content: '<?php echo base_url('blog/add_category/');?>'
			});
		}
		...
		function updateData(id) {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_blog_category_setting');?>',
				content: '<?php echo base_url('blog/edit_category/');?>' + id
			});
		}
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('blog/delete_category/');?>' + id, function(data) {
					...
					if(json.status == '<?php echo EXIT_SUCCESS;?>') {
						msg_icon = 1;
						$('#blog_category-table').DataTable().ajax.reload();
					}
