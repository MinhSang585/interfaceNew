		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="row">
					<div class="col-12">
						<div class="card">
							<?php if(permission_validation(PERMISSION_PAYMENT_GATEWAY_MAINTENANCE_ADD) == TRUE):?>
								...
								<table id="payment_gateway-table" class="table table-striped table-bordered table-hover">
									<thead>
										<tr>
											...
											<th width="40"><?php echo $this->lang->line('label_sequence');?></th>
											...
											<?php if(permission_validation(PERMISSION_PAYMENT_GATEWAY_MAINTENANCE_UPDATE) == TRUE || permission_validation(PERMISSION_PAYMENT_GATEWAY_MAINTENANCE_DELETE) == TRUE):?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#payment_gateway-table').DataTable({
				...
				"order": [[0, "asc"]],
				"ajax": {
					"url": "<?php echo site_url('paymentgateway/maintenance_listing');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function updateData(id) {
			layer.open({
				...
				content: '<?php echo base_url('paymentgateway/maintenance_edit/');?>' + id
			});
		}
		...
		function addData() {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_add_payment_gateway_maintenance');?>',
				content: '<?php echo base_url('paymentgateway/maintenance_add/');?>'
			});
		}
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('paymentgateway/maintenance_delete/');?>' + id, function(data) {
