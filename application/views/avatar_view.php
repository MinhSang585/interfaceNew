					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<form action="<?php echo site_url('avatar/search');?>" id="avatar-form" name="avatar-form" class="form-horizontal" method="post" accept-charset="utf-8" novalidate="novalidate">
									<div class="form-group row">
										<div class="col-md-3">
											...
											<div class="row mb-2">
												<label class="col-4 col-form-label col-form-label-sm font-weight-normal"><?php echo $this->lang->line('label_avatar_name');?></label>
												<div class="col-8">
													<input type="text" class="form-control form-control-sm" id="avatar_name" name="avatar_name" value="">
												</div>
											</div>										
										</div>
										<div class="col-md-3">
							...
							<!-- /.card-header -->
							<?php if(permission_validation(PERMISSION_AVATAR_ADD) == TRUE):?>
							<div class="card-header">
							...
							<?php endif;?>
							<div class="card-body">
								<table id="avatar-table" class="table table-striped table-bordered table-hover" style="width:100%;">
									<thead>
										<tr>
											<th><?php echo $this->lang->line('label_hashtag');?></th>
											<th><?php echo $this->lang->line('label_avatar_name');?></th>
											...
											<?php if(permission_validation(PERMISSION_AVATAR_UPDATE) == TRUE OR permission_validation(PERMISSION_AVATAR_DELETE) == TRUE):?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
					...
					$.ajax({url: form.attr('action'),
						...
						success: function (data) {
							...
							if(json.status == '<?php echo EXIT_SUCCESS;?>') {
								$('#avatar-table').DataTable().ajax.reload();
							}
							else {
			...
			$('#avatar-table').DataTable({
				...
				"order": [[0, "desc"]],
				"ajax": {
					"url": "<?php echo site_url('avatar/listing');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function addData() {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_add_avatar');?>',
				content: '<?php echo base_url('avatar/add/');?>'
			});
		}
		...
		function updateData(id) {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_avatar_setting');?>',
				content: '<?php echo base_url('avatar/edit/');?>' + id
			});
		}
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('avatar/delete/');?>' + id, function(data) {
					var json = JSON.parse(JSON.stringify(data));
