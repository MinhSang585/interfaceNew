					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<form action="<?php echo site_url('message/search');?>" id="message-form" name="message-form" class="form-horizontal" method="post" accept-charset="utf-8" novalidate="novalidate">
									<div class="form-group row">
										<div class="col-md-3">
											...
											<div class="row mb-2">
												<label class="col-4 col-form-label col-form-label-sm font-weight-normal"><?php echo $this->lang->line('label_message_title');?></label>
												<div class="col-8">
										...
										<div class="col-md-3">
											<div class="row mb-2">
												...
												<div class="col-8">
													<select class="form-control form-control-sm select2bs4 col-12" id="system_message_type" name="system_message_type">
														<option value="0"><?php echo $this->lang->line('label_all');?></option>
														<?php
															foreach(get_message_type() as $k => $v)
															{
											...
											<div class="row mb-2">
												<label class="col-4 col-form-label col-form-label-sm font-weight-normal"><?php echo $this->lang->line('label_genre');?></label>
												<div class="col-8">
													<select class="form-control form-control-sm select2bs4 col-12" id="system_message_genre" name="system_message_genre">
														<option value="0"><?php echo $this->lang->line('label_all');?></option>
														<?php
															foreach(get_message_genre() as $k => $v)
															{
																echo '<option value="' . $k . '">' . $this->lang->line($v) . '</option>';
															}
														?>
							...
							<!-- /.card-header -->
							<?php if(permission_validation(PERMISSION_SYSTEM_MESSAGE_ADD) == TRUE):?>
							<div class="card-header">
								...
								<table id="message-table" class="table table-striped table-bordered table-hover" style="width:100%;">
									<thead>
										<tr>
											...
											<th><?php echo $this->lang->line('label_genre');?></th>
											...
											<?php if(permission_validation(PERMISSION_SYSTEM_MESSAGE_UPDATE) == TRUE OR permission_validation(PERMISSION_SYSTEM_MESSAGE_DELETE) == TRUE  OR permission_validation(PERMISSION_SYSTEM_MESSAGE_USER_ADD) == TRUE):?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
				...
				if(is_allowed == true) {
					...
					$.ajax({url: form.attr('action'),
						data: { 
							...
							system_message_type:  $('#system_message_type').val(),
							system_message_genre:  $('#system_message_genre').val(),
							system_message_remark : $('#system_message_remark').val(),
							status:  $('#status').val(),
						},
						type: 'post',                  
						async: 'true',
			...
			$('#message-table').DataTable({
				...
				"order": [[2, "desc"]],
				"ajax": {
					"url": "<?php echo base_url('message/listing/');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function addData() {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_add_message');?>',
				content: '<?php echo base_url('message/add/');?>'
			});
		}
		...
		function updateData(id) {
			layer.open({
				...
				content: '<?php echo base_url('message/edit/');?>' + id
			});
		}
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('message/delete/');?>' + id, function(data) {
					var json = JSON.parse(JSON.stringify(data));
					var message = json.msg;
					var msg_icon = 2;
		...
		function addUserMessageData(id){
			layer.open({
				type: 2,
				area: [((browser_width < 600) ? '100%': '100%'), ((browser_width < 600) ? '100%': '100%')],
				fixed: false,
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_message_player_add');?>',
				content: '<?php echo base_url('message/player_add/');?>'+id
			});
		}
	</script>
</body>
