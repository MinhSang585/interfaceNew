					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<form action="<?php echo site_url('blacklist/import_search');?>" id="blacklist-form" name="blacklist-form" class="form-horizontal" method="post" accept-charset="utf-8" novalidate="novalidate">
											...
											<div class="row mb-2">
												...
												<div class="col-8">
													<select class="form-control form-control-sm select2bs4 col-12" id="status" name="status">
														...
														<option value="<?php echo STATUS_ON_PENDING;?>"><?php echo $this->lang->line('status_on_pending');?></option>
							...
							<!-- /.card-header -->
							<?php if(permission_validation(PERMISSION_BLACKLIST_IMPORT_ADD) == TRUE):?>
							<div class="card-header">
								...
								<table id="blacklist-table" class="table table-striped table-bordered table-hover" style="width:100%;">
									<thead>
										<tr>
											...
											<th><?php echo $this->lang->line('label_file_name');?></th>
											...
											<?php if(permission_validation(PERMISSION_BLACKLIST_IMPORT_UPDATE) == TRUE OR permission_validation(PERMISSION_BLACKLIST_IMPORT_DELETE) == TRUE):?>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#blacklist-table').DataTable({
				...
				"order": [[0, "desc"]],
				"ajax": {
					"url": "<?php echo base_url('blacklist/import_listing/');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function addData() {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_blacklist_import_add');?>',
				content: '<?php echo base_url('blacklist/import_add/');?>'
			});
		}
		...
		function updateData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('blacklist/import_execute/');?>' + id, function(data) {
					var json = JSON.parse(JSON.stringify(data));
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('blacklist/import_delete/');?>' + id, function(data) {
					var json = JSON.parse(JSON.stringify(data));
					var message = json.msg;
					var msg_icon = 2;
