				...
				<div class="row">
					...
					<div class="col-12">
						...
						<div class="card card-primary">
							<!-- form start -->
							<?php echo form_open_multipart('bonus/update', array('id' => 'bonus-form', 'name' => 'bonus-form', 'class' => 'form-horizontal'));?>
								<div class="card-body">
									<div class="form-group row">
										<label for="bonus_name" class="col-5 col-form-label"><?php echo $this->lang->line('label_bonus_name');?></label>
										<div class="col-7">
											<input type="text" class="form-control col-10" id="bonus_name" name="bonus_name" value="<?php echo (isset($bonus_name) ? $bonus_name : '');?>">
										</div>
									</div>
									<div class="form-group row">
										<label for="bonus_seq" class="col-5 col-form-label"><?php echo $this->lang->line('label_sequence');?></label>
										<div class="col-7">
											<input type="text" class="form-control col-3" id="bonus_seq" name="bonus_seq" value="<?php echo (isset($bonus_seq) ? $bonus_seq : '');?>" maxlength="3">
										</div>
									</div>
									<div class="form-group row">
												...
												$content_html .= '<div class="tab-pane fade ' . $content_active . '" id="custom-content-below-' . $v . '" role="tabpanel" aria-labelledby="custom-content-below-' . $v . '-tab">';
													$content_html .= '<div class="form-group row mt-3">';
														...
														$content_html .= '<div class="col-7">';
															$content_html .= '<input type="text" class="form-control" id="bonus_title_' . $v . '" name="bonus_title_' . $v . '" value="'.(isset($bonus_lang[$v]['bonus_title']) ? $bonus_lang[$v]['bonus_title'] : '').'">';
														$content_html .= '</div>';
													$content_html .= '</div>';
													$content_html .= '<div class="form-group row mt-3">';
														$content_html .= '<label for="bonus_content_' . $v . '" class="col-5 col-form-label">' . $this->lang->line('label_content') . '</label>';
														$content_html .= '<div class="col-7">';
															$content_html .= '<textarea class="form-control" id="bonus_content_' . $v . '" name="bonus_content_' . $v . '" rows="4">'.(isset($bonus_lang[$v]['bonus_content']) ? $bonus_lang[$v]['bonus_content'] : '').'</textarea>';
														$content_html .= '</div>';
													$content_html .= '</div>';
													...
													$content_html .= '<div class="tab-content" id="custom-content-below-'.$v.'-tabContent">';
														...
														if(isset($bonus_lang[$v]['bonus_banner_web']) && $bonus_lang[$v]['bonus_banner_web'] != ""){
															$content_html .= '<div class="form-group row mt-3">';
																$content_html .= '<label class="col-5 col-form-label">&nbsp;</label>';
																$content_html .= '<div class="col-7">';
																	$content_html .= '<a href="'.BONUS_SOURCE_PATH . $bonus_lang[$v]['bonus_banner_web'].'" target="_blank"><img src="'.BONUS_SOURCE_PATH . $bonus_lang[$v]['bonus_banner_web'].'" width="200" border="0" /></a>';
																$content_html .= '</div>';
															$content_html .= '</div>';
														}
														...
														if(isset($bonus_lang[$v]['bonus_banner_mobile']) && $bonus_lang[$v]['bonus_banner_mobile'] != ""){
															$content_html .= '<div class="form-group row mt-3">';
																$content_html .= '<label class="col-5 col-form-label">&nbsp;</label>';
																$content_html .= '<div class="col-7">';
																	$content_html .= '<a href="'.BONUS_SOURCE_PATH . $bonus_lang[$v]['bonus_banner_mobile'].'" target="_blank"><img src="'.BONUS_SOURCE_PATH . $bonus_lang[$v]['bonus_banner_mobile'].'" width="200" border="0" /></a>';
																$content_html .= '</div>';
															$content_html .= '</div>';
														}
															$content_html .= '<div class="form-group row mt-3">';
								...
								<div class="card-footer">
									<input type="hidden" id="bonus_id" name="bonus_id" value="<?php echo (isset($bonus_id) ? $bonus_id : '');?>">
						...
						$.ajax({url: form.attr('action'),
							...
							success: function (data) {
								...
								if(json.status == '<?php echo EXIT_SUCCESS;?>') {
									message = json.msg;
									msg_icon = 1;
									parent.$('#uc1_' + json.response.id).html(json.response.bonus_name);
									parent.$('#uc2_' + json.response.id).html(json.response.active);
									parent.$('#uc4_' + json.response.id).html(json.response.bonus_seq);
