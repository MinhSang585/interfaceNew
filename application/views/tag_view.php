		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="row">
					<div class="col-12">
						<div class="card">
							<?php if(permission_validation(PERMISSION_TAG_ADD) == TRUE):?>
								...
								<table id="tag-table" class="table table-striped table-bordered table-hover">
									<thead>
										<tr>
											...
											<th width="80"><?php echo $this->lang->line('label_tag_code');?></th>
											<th width="100"><?php echo $this->lang->line('label_maintain_membership_limit_tag');?></th>
											<th width="150"><?php echo $this->lang->line('label_min_request_amount_win_loss');?></th>
											<th width="150"><?php echo $this->lang->line('label_max_request_amount_win_loss');?></th>
											...
											<?php if(permission_validation(PERMISSION_TAG_UPDATE) == TRUE || permission_validation(PERMISSION_TAG_DELETE) == TRUE):?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#tag-table').DataTable({
				...
				"order": [[0, "desc"]],
				"ajax": {
					"url": "<?php echo site_url('tag/listing');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function addData() {
			layer.open({
				...
				content: '<?php echo base_url('tag/add/');?>'
			});
		}
		...
		function updateData(id) {
			layer.open({
				...
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_tag_setting');?>',
				content: '<?php echo base_url('tag/edit/');?>' + id
			});
		}
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('tag/delete/');?>' + id, function(data) {
