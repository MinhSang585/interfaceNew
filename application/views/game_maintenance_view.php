		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="row">
					<div class="col-12">
						<div class="card">
							<?php if(permission_validation(PERMISSION_GAME_MAINTENANCE_ADD) == TRUE):?>
								...
								<table id="game-table" class="table table-striped table-bordered table-hover">
									<thead>
										<tr>
											...
											<th width="150"><?php echo $this->lang->line('label_frontend_display');?></th>
											...
											<?php if(permission_validation(PERMISSION_GAME_MAINTENANCE_UPDATE) == TRUE || permission_validation(PERMISSION_GAME_MAINTENANCE_DELETE) == TRUE):?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#game-table').DataTable({
				...
				"order": [[0, "asc"]],
				"ajax": {
					"url": "<?php echo site_url('game/maintenance_listing');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function updateData(id) {
			layer.open({
				...
				content: '<?php echo base_url('game/maintenance_edit/');?>' + id
			});
		}
		...
		function addData() {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_add_game_maintenance');?>',
				content: '<?php echo base_url('game/maintenance_add/');?>'
			});
		}
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('game/maintenance_delete/');?>' + id, function(data) {
