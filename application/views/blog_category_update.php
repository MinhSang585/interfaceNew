				...
				<div class="row">
					...
					<div class="col-12">
						...
						<div class="card card-primary">
							<!-- form start -->
							<?php echo form_open('blog/update_category', array('id' => 'blog_category-form', 'name' => 'blog_category-form', 'class' => 'form-horizontal'));?>
								<div class="card-body">
									<div class="form-group row">
										<label class="col-2 col-form-label"><?php echo $this->lang->line('label_blog_category');?></label>
										<div class="col-10">
											<input type="text" class="form-control" id="blog_category_name" name="blog_category_name" value="<?php echo (isset($blog_category_name) ? $blog_category_name : '');?>">
										</div>
									</div>
									<div class="form-group row">
										<label for="blog_category_pathname" class="col-2 col-form-label"><?php echo $this->lang->line('label_blog_pathname');?></label>
										<div class="col-10">
											<input type="text" class="form-control" id="blog_category_pathname" name="blog_category_pathname" value="<?php echo (isset($blog_category_pathname) ? $blog_category_pathname : '');?>">
										</div>
									</div>
									<div class="form-group row">
										<label for="meta_header" class="col-2 col-form-label"><?php echo $this->lang->line('label_seo_header');?></label>
										<div class="col-10">
											<textarea class="form-control" id="blog_category_header" name="blog_category_header" rows="25"><?php echo (isset($blog_category_header) ? $blog_category_header : '');?></textarea>
										</div>
									</div>
									<div class="form-group row">
									...
									<?php
										...
										if(sizeof($lang) > 0)
										{
											...
											foreach($lang as $k => $v)
											{
												...
												$content_html .= '<label for="announcement_name-' . $v . '" class="col-2 col-form-label">' . $this->lang->line('label_blog_category') . '</label>';
												$content_html .= '<div class="col-10">';
												$content_html .= '<input type="text" class="form-control col-12" id="blog_category_name-' . $v . '" name="blog_category_name-' . $v . '" value="' . (isset($blog_category_lang[$v]) ? $blog_category_lang[$v] : '') . '">';
												$content_html .= '</div>';
												$content_html .= '</div>';
												$content_html .= '</div>';
											}
								...
								<div class="card-footer">
									<input type="hidden" id="blog_category_id" name="blog_category_id" value="<?php echo (isset($blog_category_id) ? $blog_category_id : '');?>">
						...
						$.ajax({url: form.attr('action'),
							...
							success: function (data) {
								...
								if(json.status == '<?php echo EXIT_SUCCESS;?>') {
									message = json.msg;
									msg_icon = 1;
									parent.$('#uc1_' + json.response.id).html(json.response.blog_category_name);
									parent.$('#uc2_' + json.response.id).html(json.response.active);
									parent.$('#uc3_' + json.response.id).html(json.response.blog_category_pathname);
									...
									if(json.msg.blog_pathname_error != '') {
