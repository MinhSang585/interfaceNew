					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<form action="<?php echo site_url('report/player_risk_search');?>" id="report-form" name="report-form" class="form-horizontal" method="post" accept-charset="utf-8" novalidate="novalidate">
									<div class="form-group row">
										...
										<div class="col-md-3">
											<div class="row mb-2">
												...
												<div class="col-8">
													<select class="form-control form-control-sm select2bs4 col-12" id="suspended" name="suspended">
														<option value="0"><?php echo $this->lang->line('label_all');?></option>
														<option value="<?php echo STATUS_UNSUSPEND;?>"><?php echo $this->lang->line('status_unsuspend');?></option>
										...
										<div class="col-md-3">
											<div class="row mb-2">
												<label class="col-4 col-form-label col-form-label-sm font-weight-normal"><?php echo $this->lang->line('label_percentage');?></label>
												<div class="col-8">
													<select class="form-control form-control-sm select2bs4 col-12" id="percentage" name="percentage">
														<option value="0"><?php echo $this->lang->line('label_all');?></option>
														<?php 
															if(isset($miscellaneous['risk_announcement_rate'])){
																$risk_announcement_rate_data = array_values(array_filter(explode(',', $miscellaneous['risk_announcement_rate'])));
																if(sizeof($risk_announcement_rate_data)>0){
																	foreach($risk_announcement_rate_data as $risk_announcement_rate_data_row){
																		echo "<option value='".$risk_announcement_rate_data_row."'>".$risk_announcement_rate_data_row."</option>";
																	}
																}
															}
														?>
											...
											<div class="row mb-2">
												...
												<div class="col-8">
													<input type="text" class="form-control form-control-sm" id="player_risk_id" name="player_risk_id" value="<?php echo (isset($data_search['player_risk_id']) ? $data_search['player_risk_id'] : '');?>">
												</div>
											</div>
										</div>
										<div class="col-md-3">
							...
							<?php echo form_open('export/player_risk_export_excel', 'class="export" id="export_form"');?>
							<?php echo form_close(); ?>
							<!-- /.card-header -->
							<?php endif;?>
							<!-- /.card-header -->
								...
								<table id="report-table" class="table table-striped table-bordered table-hover">
									<thead>
										<tr>
											...
											<th width="130"><?php echo $this->lang->line('label_percentage');?></th>
											<th width="130"><?php echo $this->lang->line('label_total_win_loss');?></th>
											<th width="120"><?php echo $this->lang->line('label_win_loss_suspended');?></th>
										</tr>
									</thead>
									<tbody></tbody>
								</table>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
				...
				if(is_allowed == true) {
					...
					$.ajax({url: form.attr('action'),
						data: { 
								...
								player_risk_id : $('#player_risk_id').val(),
						},
						type: 'post',                  
						async: 'true',
						beforeSend: function() {
			...
			$('#report-table').DataTable({
				...
				"order": [[0, "desc"]],
				"ajax": {
					"url": "<?php echo site_url('report/player_risk_listing');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function exportData(){
			$.ajax({url: '<?php echo base_url("export/player_risk_export_excel_check");?>',
				type: 'get',								
				async: 'true',
				beforeSend: function() {
					layer.load(1);
