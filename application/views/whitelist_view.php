					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								<form action="<?php echo site_url('whitelist/search');?>" id="whitelist-form" name="whitelist-form" class="form-horizontal" method="post" accept-charset="utf-8" novalidate="novalidate">
									<div class="form-group row">
										...
										<div class="col-md-3">
											...
											<div class="row mb-2">
												<label class="col-4 col-form-label col-form-label-sm font-weight-normal"><?php echo $this->lang->line('label_white_list_value');?></label>
												<div class="col-8">
													<input type="text" class="form-control form-control-sm" id="value" name="value" value="">
												</div>
											</div>
										</div>
										<div class="col-md-3">
							...
							<!-- /.card-header -->
							<?php if(permission_validation(PERMISSION_WHITELIST_ADD) == TRUE):?>
							<div class="card-header">
							...
							<?php endif;?>
							<div class="card-body">
								<table id="whitelist-table" class="table table-striped table-bordered table-hover" style="width:100%;">
									<thead>
										<tr>
											...
											<th><?php echo $this->lang->line('label_white_list_value');?></th>
											...
											<?php if(permission_validation(PERMISSION_WHITELIST_UPDATE) == TRUE OR permission_validation(PERMISSION_WHITELIST_DELETE) == TRUE):?>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
					...
					$.ajax({url: form.attr('action'),
						...
						success: function (data) {
							...
							if(json.status == '<?php echo EXIT_SUCCESS;?>') {
								$('#whitelist-table').DataTable().ajax.reload();
							}
							else {
			...
			$('#whitelist-table').DataTable({
				...
				"order": [[0, "desc"]],
				"ajax": {
					"url": "<?php echo base_url('whitelist/listing/');?>",
					"dataType": "json",
					"type": "POST",
					"data": function (d) {
		...
		function addData() {
			layer.open({
				...
				content: '<?php echo base_url('whitelist/add/');?>'
			});
		}
		...
		function updateData(id) {
			layer.open({
				...
				maxmin: true,
				scrollbar: false,
				title: '<?php echo $this->lang->line('title_whitelist_setting');?>',
				content: '<?php echo base_url('whitelist/edit/');?>' + id
			});
		}
		...
		function deleteData(id) {
			layer.confirm('<?php echo $this->lang->line('label_confirm_to_proceed');?>', {
				...
				$.get('<?php echo base_url('whitelist/delete/');?>' + id, function(data) {
					var json = JSON.parse(JSON.stringify(data));
