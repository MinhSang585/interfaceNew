				...
				<div class="row">
					...
					<div class="col-12">
						...
						<div class="card card-primary">
							<!-- form start -->
							<?php echo form_open('paymentgateway/maintenance_update', array('id' => 'paymentgateway-form', 'name' => 'paymentgateway-form', 'class' => 'form-horizontal'));?>
								<div class="card-body">
									<div class="form-group row">
										<label class="col-5 col-form-label"><?php echo $this->lang->line('label_name');?></label>
										<div class="col-7">
											<select class="form-control select2bs4 col-10" id="payment_gateway_id" name="payment_gateway_id">
												<option value=""><?php echo $this->lang->line('place_holder_game_name');?></option>
												<?php
													if(isset($payment_gateway)){
														if(sizeof($payment_gateway)>0){
															foreach($payment_gateway as $row){
																if($row['payment_gateway_id'] == $payment_gateway_id){
																	echo '<option value="' . $row['payment_gateway_id'] . '" selected>' . (isset($row['payment_gateway_name']) ? $this->lang->line($row['payment_gateway_name'])." (".$this->lang->line(get_payment_gateway_type($row['payment_gateway_type_code'])).")" : '_') . '</option>';
																}else{
																	echo '<option value="' . $row['payment_gateway_id'] . '">' . (isset($row['payment_gateway_name']) ? $this->lang->line($row['payment_gateway_name'])." (".$this->lang->line(get_payment_gateway_type($row['payment_gateway_type_code'])).")" : '_') . '</option>';
																}
															}
														}
													}
									...
									<div class="form-group row">
										...
										<div class="col-7">
											<input type="checkbox" id="is_maintenance" name="is_maintenance" value="1" <?php echo ((isset($is_maintenance) && $is_maintenance == STATUS_YES) ? 'checked' : '');?> data-bootstrap-switch data-off-color="secondary" data-on-color="success">
										</div>
									</div>
									<div class="form-group row">
										<label for="is_front_end_display" class="col-5 col-form-label"><?php echo $this->lang->line('label_frontend_display');?></label>
										<div class="col-7">
											<input type="checkbox" id="is_front_end_display" name="is_front_end_display" value="1" <?php echo ((isset($is_front_end_display) && $is_front_end_display == STATUS_YES) ? 'checked' : '');?> data-bootstrap-switch data-off-color="secondary" data-on-color="success">
										</div>
									</div>
									<div class="form-group row">
										<label for="fixed_maintenance" class="col-5 col-form-label"><?php echo $this->lang->line('label_fixed_maintenance');?></label>
										<div class="col-7">
											<input type="checkbox" id="fixed_maintenance" name="fixed_maintenance" value="1" <?php echo ((isset($fixed_maintenance) && $fixed_maintenance == STATUS_YES) ? 'checked' : '');?> data-bootstrap-switch data-off-color="secondary" data-on-color="success">
										</div>
									</div>
									<div class="form-group row">
									...
									<div class="form-group row">
										...
										<div class="col-7">
											<div class="input-group date" id="fixed_from_time_click" data-target-input="nearest">
												<input type="text" id="fixed_from_time" name="fixed_from_time" class="form-control col-6 datetimepicker-input" value="<?php echo (isset($fixed_from_time) ? $fixed_from_time : '');?>" data-target="#fixed_from_time_click"/>
									...
									<div class="form-group row">
										...
										<div class="col-7">
											<div class="input-group date" id="fixed_to_time_click" data-target-input="nearest">
												<input type="text" id="fixed_to_time" name="fixed_to_time" class="form-control col-6 datetimepicker-input" value="<?php echo (isset($fixed_to_time) ? $fixed_to_time : '');?>" data-target="#fixed_to_time_click"/>
									...
									<div class="form-group row">
										...
										<div class="col-7">
											<input type="checkbox" id="urgent_maintenance" name="urgent_maintenance" value="1" <?php echo ((isset($urgent_maintenance) && $urgent_maintenance == STATUS_YES) ? 'checked' : '');?> data-bootstrap-switch data-off-color="secondary" data-on-color="success">
										</div>
									</div>
									<div class="form-group row">
										...
										<div class="col-7">
											<div class="input-group date" id="urgent_date_click" data-target-input="nearest">
												<input type="text" id="urgent_date" name="urgent_date" class="form-control col-6 datetimepicker-input" value="<?php echo (isset($urgent_date) ? date('Y-m-d H:i', $urgent_date) : '');?>" data-target="#urgent_date_click"/>
								...
								<div class="card-footer">
									<input type="hidden" id="payment_gateway_maintenance_id" name="payment_gateway_maintenance_id" value="<?php echo (isset($payment_gateway_maintenance_id) ? $payment_gateway_maintenance_id : '');?>">
						...
						$.ajax({url: form.attr('action'),
							...
							success: function (data) {
								...
								if(json.status == '<?php echo EXIT_SUCCESS;?>') {
									...
									parent.$('#uc11_' + json.response.id).html(json.response.is_front_end_display);
									parent.$('#uc12_' + json.response.id).html(json.response.game_name);
