...
class Paymentgateway extends MY_Controller {
...

			{
...

				{
...

					{
						...
						case STATUS_YES: $row[] = '<span class="badge bg-success" id="uc2_' . $post->payment_gateway_id . '">' . $this->lang->line('status_yes') . '</span>'; break;

						default: $row[] = '<span class="badge bg-secondary" id="uc2_' . $post->payment_gateway_id . '">' . $this->lang->line('status_no') . '</span>'; break;
...

					{
						...
						case STATUS_YES: $row[] = '<span class="badge bg-success" id="uc3_' . $post->payment_gateway_id . '">' . $this->lang->line('status_yes') . '</span><br />'; break;

						default: $row[] = '<span class="badge bg-secondary" id="uc3_' . $post->payment_gateway_id . '">' . $this->lang->line('status_no') . '</span><br />'; break;

					}

					

					$row[] = '<span id="uc4_' . $post->payment_gateway_id . '">' . (($post->fixed_day > 0) ? $this->lang->line(get_day($post->fixed_day)) : '-') . '</span><br />';

					$row[] = '<span id="uc5_' . $post->payment_gateway_id . '">' . (( ! empty($post->fixed_from_time)) ? $post->fixed_from_time : '-') . '</span><br />';

					$row[] = '<span id="uc6_' . $post->payment_gateway_id . '">' . (( ! empty($post->fixed_to_time)) ? $post->fixed_to_time : '-') . '</span>';
...

					{
						...
						case STATUS_YES: $row[] = '<span class="badge bg-success" id="uc7_' . $post->payment_gateway_id . '">' . $this->lang->line('status_yes') . '</span><br />'; break;

						default: $row[] = '<span class="badge bg-secondary" id="uc7_' . $post->payment_gateway_id . '">' . $this->lang->line('status_no') . '</span><br />'; break;

					}

					

					$row[] = '<span id="uc8_' . $post->payment_gateway_id . '">' . (($post->urgent_date > 0) ? date('Y-m-d H:i:s', $post->urgent_date) : '-') . '</span>';
	...
	public function add(){
		...
		{
			$this->load->view('payment_gateway_add');
		}
...

				{
...

					{
...

						{
							...
							$this->user_model->insert_log(LOG_PAYMENT_GATEWAY_ADD, $newData);
...

						{
							...
							$this->account_model->insert_log(LOG_PAYMENT_GATEWAY_ADD, $newData);
